...
void update_H(double **W, double **H, double **V, int row, int k, int col)
{
    ...
    multiply(denominator, transpose_WWH, D, k, col, row);//(WT*W)*HD
...
void update_W(double **W, double **H, double **V, int row, int k, int col)
{
    ...
    for (int i = 0; i < col; i++)
        VD[i] = (double *)malloc(row * sizeof(double));
    ...
    cout<<"DW matrix is:"<<endl;
    print_matrix(D,row,col);

    transpose(H, HT, k, col); // HT
    multiply(VD, V, D, row, col, col); // V*HT
    multiply(numerator, VD, HT, row, col, k); // V*HT
