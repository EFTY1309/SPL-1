...
void update_W2(double **W, double **H, double **V, int row, int k, int col)
{
    ...
    transpose(H, HT, k, col); // HT // V*HT
    multiply(numerator, V, HT, row, col, k); // VHT
...
void update_H2(double **W, double **H, double **V, int row, int k, int col)
{
...
void LSNMF()
{
    ...
    while (cost > EPSILON)
    {
        ...
        if ((counter % 2) == 0)
        {
            update_H2(W, H, matrix, row, k, col);
            //cout << "New H" << endl;
        }
        else
        {
             update_W2(W, H, matrix, row, k, col);
           // cout << "New W" << endl;
